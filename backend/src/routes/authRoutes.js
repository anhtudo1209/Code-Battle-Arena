...
router.get("/github/callback", async (req, res) => {
  ...
  try {
    ...
    const primaryEmail = emails.find(e => e.primary)?.email || `${githubUser.login}@github.com`;

    // get or create user in our database
    let userId;
    const getOAuth = await query(
      "SELECT * FROM oauth_accounts WHERE provider = $1 AND provider_user_id = $2",
      ["github", githubUser.id]
    );
    if (getOAuth.rows.length) {
      userId = getOAuth.rows[0].user_id;
    } else {
      const getUser = await query("SELECT * FROM users WHERE email = $1", [primaryEmail]);
      if (getUser.rows.length) {
        userId = getUser.rows[0].id;
      } else {
        const result = await query(
          "INSERT INTO users (username, email, password) VALUES ($1, $2, $3) RETURNING id",
          [githubUser.login, primaryEmail, ""]
        );
    ...
    // generate JWT tokens
    const accessTokenJWT = jwt.sign({ id: userId }, process.env.ACCESS_TOKEN_SECRET, { expiresIn: "1h" });
    const refreshTokenJWT = jwt.sign({ id: userId, type: 'refresh' }, process.env.REFRESH_TOKEN_SECRET, { expiresIn: "7d" });

    await query(
      "INSERT INTO refresh_tokens (user_id, token, expires_at) VALUES ($1, $2, $3)",
      [userId, refreshTokenJWT, new Date(Date.now() + 7*24*60*60*1000)]
    );
        ...
        <body>
          <script>
            window.opener.postMessage(
              { user: ${JSON.stringify(githubUser)}, accessToken: "${accessTokenJWT}", refreshToken: "${refreshTokenJWT}" },
              "http://localhost:5173"
            );
            window.close();
          </script>
