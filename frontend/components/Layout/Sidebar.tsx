...
export default function Sidebar({ onNavigate }: SidebarProps) {
    ...
    useEffect(() => {
        ...
        }).catch(err => console.error("Failed to fetch sidebar user", err));
    }, []);
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-0 bg-ui-800 relative">
                ...
                {activeTab === 'streak' && (
                    <div className="p-4 space-y-4 animate-fade-in">
                        ...
                        <div className="bg-ui-900 border border-ui-border">
                            <StreakCard currentStreak={user?.daily_streak || 0} />
                        </div>

                        <div className="grid grid-cols-1 gap-2 mt-4">
                            <div className="bg-ui-900 border border-ui-border p-3 flex flex-col items-center justify-center">
                                <span className="text-2xl font-display font-bold text-ui-text-main">{user?.max_streak || user?.daily_streak || 0}</span>
                                <span className="text-[9px] text-gray-500 uppercase tracking-wide">All-Time Max Streak</span>
                            </div>
                        </div>
                    </div>
                )}
