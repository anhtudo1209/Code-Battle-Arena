...
// --- MOCK SERVICES (Internal for Demo) ---
const mockLogin = async (creds: any) => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      if (creds.username.toLowerCase() === 'error') reject(new Error("Invalid credentials."));
      else resolve({ token: "cyber-token-123" });
    }, 1500);
  });
};

const mockRegister = async (creds: any) => {
    return new Promise((resolve) => {
        setTimeout(() => resolve({ token: "new-user-token" }), 1500);
    });
};
...
export default function LoginView({ onLoginSuccess }: LoginViewProps) {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
        await mockRegister({ username: regUsername, email: regEmail, password: regPassword });
  ...
  const handleForgotSubmit = async (e: React.FormEvent) => {
    ...
    setForgotMsg("");
    try {
        await mockForgotPass(forgotEmail);
        setEmailSent(true);
