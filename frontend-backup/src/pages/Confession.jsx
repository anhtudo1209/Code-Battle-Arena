...
const MOCK_CONFESSIONS = [
  {
    id: "CF-9021",
    content:
      "I secretly judge people who use light mode in their IDE. It's like staring into the sun. How do you even code like that without burning your retinas?",
    ...
    commentsList: [
      {
        user: "Dev4Life",
        text: "My eyes burn just thinking about it.",
        time: "1h ago",
      },
      {
        user: "SolarFlare",
        text: "I use light mode to stay awake.",
        time: "30m ago",
      },
    ],
  ...
  },
  {
    id: "CF-9020",
    content:
      "I pushed directly to production on a Friday at 4:55 PM. The deployment pipeline failed, but I just closed my laptop and went home. The guilt is eating me alive.",
    ...
    commentsList: [
      {
        user: "TechLead",
        text: "So THAT'S why pagerduty went off.",
        time: "4h ago",
      },
      { user: "JuniorDev", text: "Legend.", time: "3h ago" },
      {
        user: "SysAdmin",
        text: "Please don't come to work on Monday.",
        time: "2h ago",
      },
    ],
  ...
  },
  {
    id: "CF-9019",
    content:
      "Sometimes I use ChatGPT to write simple regex because my brain refuses to learn it properly. I've been a dev for 6 years.",
    timestamp: "1 day ago",
    likes: 892,
    commentsCount: 15,
    commentsList: [
      {
        user: "RegexWizard",
        text: "Does anyone actually learn regex?",
        time: "20h ago",
      },
    ],
...
export default function Confession() {
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: "Code Battle Arena Confession",
          text: content,
  ...
  const filteredConfessions = sortedConfessions.filter(
    (c) =>
      !searchQuery ||
        ...
        <div className="max-w-5xl mx-auto flex flex-col">
          ...
          <div className="mb-8 mt-4">
            ...
            <p className="text-slate-400 max-w-xl text-sm md:text-base leading-relaxed">
              Anonymous confessions from the battlefield. Share your
              deployment disasters, spaghetti code shame, and impostor
              syndrome secrets.
            </p>
          </div>

          {/* Filter Bar */}
          <div className="bg-slate-900/60 backdrop-blur-xl border border-slate-800 rounded-2xl p-4 mb-6 shadow-xl flex-shrink-0">
            <div className="flex flex-col md:flex-row gap-4 justify-between">
              <div className="flex bg-slate-950/80 p-1 rounded-xl border border-slate-800 w-full md:w-auto">
                ...
                <button
                  ...
                  className="flex-1 md:flex-none px-4 py-2 rounded-lg text-sm font-medium bg-emerald-600 hover:bg-emerald-500 text-white transition-all flex items-center justify-center gap-2"
                >
          ...
          {/* Confessions Feed - Scrollable */}
          <div className="flex-1 overflow-y-auto pb-6">
            <div className="grid gap-6">
            {filteredConfessions.map((confession) => (
              <div
                key={confession.id}
                className="w-full bg-slate-900/60 border border-slate-800 rounded-2xl p-5 hover:border-slate-700 transition-all duration-300 group hover:shadow-lg hover:shadow-emerald-900/10"
              >
                {/* Card Header */}
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-10 h-10 rounded-full flex items-center justify-center text-lg font-bold text-white shadow-inner ${confession.bgClass}`}
                    >
                ...
                <div className="mb-4">
                  <p
                    className={`text-slate-200 leading-relaxed whitespace-pre-line break-words ${
                ...
                <div className="flex flex-wrap gap-2 mb-5">
                  {confession.tags.map((tag) => (
                    <span
                      key={tag}
                      className="text-[10px] md:text-xs font-medium text-emerald-400 bg-emerald-950/30 px-2 py-1 rounded border border-emerald-900/50 hover:border-emerald-700/50 cursor-pointer transition-colors"
                    >
                      #{tag}
                    </span>
                  ))}
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4 md:gap-6">
                    ...
                    >
                      <Heart
                        className={`w-5 h-5 ${
                          likedPosts[confession.id] ? "fill-current" : ""
                        }`}
                      />
                      <span className="text-sm font-medium">
                        {confession.likes + (likedPosts[confession.id] ? 1 : 0)}
                      </span>
                    </button>
                    <button
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button
                      onClick={() => handleShare(confession.content)}
                ...
                {expandedComments[confession.id] && (
                  <div className="flex flex-col mt-4 pt-4 border-t border-slate-800/50">
                    <div className="flex flex-col gap-2 max-h-60 overflow-y-auto mb-4 pr-1">
                      {confession.commentsList.length > 0 ? (
                        confession.commentsList.map((comment, idx) => (
                          <div key={idx} className="flex gap-3 mb-3">
                            <img
                              src={`https://ui-avatars.com/api/?name=${comment.user}&background=random&color=fff`}
                            ...
                            <div className="flex flex-col max-w-[85%]">
                              ...
                              <span className="text-[10px] text-slate-500 mt-1 ml-2">
                                {comment.time}
                              </span>
                            </div>
                          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-slate-950/80 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-700 w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden">
            ...
            <div className="p-6 pt-2 flex justify-end gap-3 bg-slate-900/50">
              <button
                onClick={() => setIsModalOpen(false)}
                className="px-5 py-2.5 text-slate-300 font-medium hover:text-white transition-colors text-sm"
              >
                Cancel
              </button>
              <button
